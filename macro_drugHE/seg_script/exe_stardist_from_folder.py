...
#     show_image(labels, cmap=cmap)
    ...
    imsave(os.path.join(output_dir,output_fn + '_SEG.tif'), labels, compress=ZIP_DEFLATED)
    export_imagej_rois(os.path.join(output_dir,output_fn + '_SEG_ROI'), polys['coord'], compression=ZIP_DEFLATED)
    #show_image(output_dir, output_fn+'_model_'+model_name+'_demo',labels, cmap=cmap)
    
def run_stardist(input_dir, output_dir, 
...
#         print("Normalizing image channels %s." % ('jointly' if axis_norm is None or 2 in axis_norm else 'independently'))
    ...
    for image_fn in files:
        print(image_fn)             
        output_fn = image_fn[:-8]
        print(output_fn)  
        obs_img = imread(os.path.join(r, image_fn))
...
#     print(args['datatag'])
    run_stardist(args['input_dir'], args['output_dir'], args['trained_model'], args['output_fn'], args['is_normalized'])
